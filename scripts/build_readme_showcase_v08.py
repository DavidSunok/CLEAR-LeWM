#!/usr/bin/env python3
"""Build the v0.8 README hero and headline benchmark graphic."""
...
class _ScaledDraw:
    ...
    def _scaled_font(self, text_font):
        return ImageFont.truetype(
            text_font.path,
            size=text_font.size * self._scale,
        )

    def rectangle(self, box, **kwargs):
        if "width" in kwargs:
            kwargs["width"] *= self._scale
        return self._draw.rectangle(self._coords(box), **kwargs)

    def rounded_rectangle(self, box, *, radius=0, **kwargs):
        if "width" in kwargs:
            kwargs["width"] *= self._scale
    ...
    def line(self, xy, *, width=0, **kwargs):
...
def _task_images() -> dict[str, Image.Image]:
    images = {}
    for task in TASKS:
        image = Image.open(ASSETS / "task_gifs" / f"{task}_v08.gif")
...
def _draw_hero() -> None:
    ...
    scale = 2
    canvas = Image.new("RGB", (width * scale, height * scale), "#0B1220")
    draw = _ScaledDraw(canvas, scale)
    ...
    output = ASSETS / "readme_hero_v08_fast.png"
...
def _success_rate(task: str, tier: str, policy: str) -> float:
    values = []
    for seed in (0, 1, 42):
        path = RESULTS / "runs" / policy / f"{task}-{tier}-seed{seed}.json"
        values.append(
...
def _draw_results() -> None:
    ...
    output = ASSETS / "headline_results_v08.png"
    canvas.save(output, optimize=True)
...
def main() -> int:
    ...
    _draw_hero()
    _draw_results()
    print(ASSETS / "readme_hero_v08_fast.png")
    print(ASSETS / "headline_results_v08.png")
    return 0
