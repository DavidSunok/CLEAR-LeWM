#!/usr/bin/env python3
"""Build the v0.8 Moderate/Strict film and four task GIFs."""
...
ASSETS = ROOT / "assets"
TWOROOM_RESULT = (
    ROOT / "results" / "v0.8" / "runs" / "official-lewm" / "tworoom-strict-seed42.json"
)
...
# Planning results reported in Fig. 6 of the LeWM paper. The three labels match
# the audited chart so the film can compare protocols without changing models.
PAPER_BENCHMARK = {
    "PushT": {
        "Official LeWM": 96.0,
        "DINOv2 No-Proprio": 74.0,
        "GCBC Joint": 75.0,
    },
    "Cube": {
        "Official LeWM": 74.0,
        "DINOv2 No-Proprio": 86.0,
        "GCBC Joint": 84.0,
    },
    "TwoRoom": {
        "Official LeWM": 87.0,
        "DINOv2 No-Proprio": 100.0,
        "GCBC Joint": 100.0,
    },
}
PAPER_MODEL_COLORS = {
    "Official LeWM": "#E85F52",
    "DINOv2 No-Proprio": "#416FBD",
    "GCBC Joint": "#8DB5DF",
}


def matched_benchmark() -> dict:
    model_labels = {
        "official-lewm": "Official LeWM",
        "dinov2-no-proprio-lewm": "DINOv2 No-Proprio",
        "gcbc-joint-lewm": "GCBC Joint",
    }
    benchmark = {
        mode: {LABELS[task]: {} for task in ("pusht", "cube", "tworoom")}
        for mode in ("moderate", "strict")
    }
    for row in json.loads(SUMMARY.read_text())["rows"]:
        if row["task"] == "reacher":
            continue
        benchmark[row["protocol"]][LABELS[row["task"]]][model_labels[row["model"]]] = (
            float(row["success_rate_mean_percent"])
        )
    return benchmark
...
def intro_frame() -> Image.Image:
    ...
    draw.text((92, 92), "CLEAR-LeWM v0.8", font=font(72, True), fill="#FFFFFF")
    draw.text(
        (96, 206), "WHEN SUCCESS MEANS COMPLETION", font=font(39, True), fill=PURPLE
    )
    draw.text(
        (96, 282),
        "Moderate minimally repairs LeWM's original success rules.",
        font=font(39, True),
        fill="#D0D5DD",
    )
    draw.text(
        (96, 350),
        "Strict measures precise task completion.",
        font=font(39, True),
        fill="#D0D5DD",
    )
    labels = (
...
def _benchmark_frame(
    ...
    legend = tuple(PAPER_MODEL_COLORS.items())
    for index, (model, color) in enumerate(legend):
        x = 940 + index * 290
        draw.rectangle((x, 220, x + 22, 242), fill=color)
        draw.text((x + 34, 216), model, font=font(17, True), fill="#344054")

    plot_left, plot_right = 150, 1770
    plot_top, plot_bottom = 302, 786
    plot_height = plot_bottom - plot_top
    for tick in (0, 25, 50, 75, 100):
        y = plot_bottom - tick / 100 * plot_height
        draw.line((plot_left, y, plot_right, y), fill="#D6DEDA", width=2)
        draw.text((104, y - 11), str(tick), font=font(16), fill="#718078")
    ...
    for group_x, (task, values) in zip(
        ...
        for model_index, (model, value) in enumerate(values.items()):
            x0 = start_x + model_index * (bar_width + bar_gap)
            ...
            if eased > 0.08:
                near_ceiling = y0 < plot_top + 48
                centered(
                    draw,
                    (
                        x0 + bar_width / 2,
                        y0 + 25 if near_ceiling else y0 - 24,
                    ),
                    f"{value:.0f}",
                    font(22, True),
                    "#FFFFFF" if near_ceiling else PAPER_MODEL_COLORS[model],
                )
        centered(draw, (group_x, 834), task, font(27, True), INK)
...
def audit_findings_frame(progress: float) -> Image.Image:
    ...
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, VIDEO_WIDTH, 164), fill="#0B1220")
    draw.text(
        (66, 24),
    ...
    draw.text(
        (66, 62),
        "The evaluator can award success before the task is complete.",
        font=font(42, True),
        fill="#FFFFFF",
    )
    ...
    findings = (
        (
            "01",
            "TRIVIAL START / GOAL PAIRS",
            "A policy can begin inside the success set, inflating SR before planning.",
        ),
        (
            "02",
            "REACHER ANGLE TOPOLOGY",
            "Linear subtraction mis-scores a periodic shoulder configuration.",
        ),
        (
            "03",
            "TWOROOM COLLISION / DATA",
            "Endpoint checks miss wall crossings; released data contain "
            "invalid transitions.",
        ),
        (
            "04",
            "PERMISSIVE TASK PROXIES",
            "A success flag need not prove object, endpoint, or legal-route "
            "completion.",
        ),
    )
    boxes = (
    ...
    active_index = min(int(progress * len(findings)), len(findings) - 1)
    for index, (box, (number, title, detail)) in enumerate(zip(boxes, findings)):
        x0, y0, x1, y1 = box
        ...
        rounded(
            ...
            box,
            "#FFFFFF",
            RED if active else "#CDD6E0",
            4 if active else 2,
            8,
        )
        draw.rectangle((x0, y0, x0 + 15, y1), fill=RED)
        draw.text((x0 + 42, y0 + 37), number, font=font(55, True), fill="#E88980")
        draw.text((x0 + 154, y0 + 46), title, font=font(29, True), fill=INK)
        draw_wrapped(
            draw,
            (x0 + 154, y0 + 128),
            detail,
    ...
    draw.text(
        (66, 950),
        "The benchmark therefore needs two explicit answers, not one ambiguous SR.",
        font=font(29, True),
        fill=INK,
    )
    return image
...
def _large_mode_panel(
    ...
    rounded(
        ...
        box,
        "#FFFFFF",
        color if active else "#CBD5DF",
        5 if active else 2,
        8,
    )
    draw.rectangle((x0, y0, x1, y0 + 14), fill=color)
    rounded(draw, (x0 + 38, y0 + 40, x0 + 292, y0 + 88), tint, radius=24)
    centered(draw, (x0 + 165, y0 + 64), eyebrow, font(17, True), color)
    draw.text((x0 + 38, y0 + 126), name, font=font(52, True), fill=color)
    draw_wrapped(
        draw,
        (x0 + 38, y0 + 208),
        statement,
    ...
    for index, bullet in enumerate(bullets):
        y = y0 + 348 + index * 91
        draw.ellipse((x0 + 42, y + 7, x0 + 59, y + 24), fill=color)
        draw.text((x0 + 82, y), bullet, font=font(27), fill="#344054")


def protocol_modes_frame(progress: float) -> Image.Image:
    image = Image.new("RGB", (VIDEO_WIDTH, VIDEO_HEIGHT), "#EEF2F6")
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, VIDEO_WIDTH, 164), fill="#0B1220")
    draw.text(
    ...
    draw.text(
        (66, 62),
        "Two calibrated modes. Two precise scientific questions.",
        font=font(42, True),
        fill="#FFFFFF",
    )
    _large_mode_panel(
        ...
        "MODERATE",
        "MINIMAL REPAIR",
        "Preserve LeWM's original task intent.",
        BLUE,
        BLUE_BG,
        (
            "Fix Reacher periodic topology",
            "Fix TwoRoom swept-disk collision",
            "Remove trivial initial pairs",
            "Retain released task semantics",
        ),
        active=progress < 0.52,
    )
    _large_mode_panel(
        draw,
        (1002, 220, 1854, 918),
        "STRICT",
        "SEMANTIC COMPLETION",
        "Measure precise, task-relevant completion.",
        GREEN,
        GREEN_BG,
        (
            "Score T / cube / fingertip / route",
            "Use tighter physical tolerances",
            "Require short persistence",
            "Verify topology and legal motion",
        ),
        active=progress >= 0.52,
    )
    centered(draw, (960, 560), "→", font(48, True), GREEN)
    draw.text(
        (66, 972),
        "Moderate restores comparability. Strict raises the completion contract.",
        font=font(29, True),
        fill=INK,
    )
    return image
...
def matched_v08_frame(progress: float) -> Image.Image:
    benchmark = matched_benchmark()
    source = "CLEAR-LeWM v0.8  |  seeds 0, 1, 42  |  mean SR  |  pure CEM 300 x 30"
    if progress < 0.12:
        return _benchmark_frame(
            PAPER_BENCHMARK,
            1.0 - progress / 0.12,
...
def comparison_frame(
    ...
    draw.text((1170, 42), "v0.8 PROTOCOL", font=font(18, True), fill="#98A2B3")
    ...
    _comparison_panel(
        draw, left, "v0.8 MODERATE", left_titles[task], BLUE, "MINIMAL FIX", BLUE_BG
    )
    _comparison_panel(
        draw, right, "v0.8 STRICT", right_titles[task], GREEN, "SEMANTIC", GREEN_BG
    )
...
def build_overview(
    ...
    for index in range(6 * fps):
        emit(matched_v08_frame(index / (6 * fps - 1)))
    for index in range(int(3.5 * fps)):
        emit(audit_findings_frame(index / (int(3.5 * fps) - 1)))
    for index in range(4 * fps):
        emit(protocol_modes_frame(index / (4 * fps - 1)))
    for task in TASKS:
...
def _result_pair(task: str, protocol: str) -> tuple[float, float]:
    directory = ROOT / "results" / "v0.8" / "runs"
    values = {"official-lewm": [], "random": []}
    for seed in (0, 1, 42):
        for policy in values:
            result = json.loads(
                (directory / policy / f"{task}-{protocol}-seed{seed}.json").read_text()
            )
...
def main() -> int:
    ...
    if not args.overview_only:
        for task in TASKS:
            ...
            save_gif(
                frames,
                ASSETS / "task_gifs" / f"{task}_v08.gif",
                duration=67,
                colors=192,
            )
    ...
    build_overview(
        ...
        task_sources,
        {},
        showcase / "clear_lewm_v08_overview_1080p.mp4",
        showcase / "clear_lewm_v08_overview_preview.gif",
        showcase / "clear_lewm_v08_overview_poster.png",
    )
    Image.open(showcase / "clear_lewm_v08_overview_poster.png").save(
        ASSETS / "site" / "overview_poster_v08.webp", "WEBP", quality=92
    )
    if not args.overview_only:
        for path in sorted((ASSETS / "task_gifs").glob("*_v08.gif")):
            print(path)
    print(showcase / "clear_lewm_v08_overview_1080p.mp4")
    return 0
