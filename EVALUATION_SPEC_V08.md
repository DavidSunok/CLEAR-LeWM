# CLEAR-LeWM Evaluation Specification v0.8

This document is normative for both CLEAR-LeWM v0.8 protocols. A result may
use the `v0.8 Moderate` or `v0.8 Strict` label only when it uses the matching
checked-in manifest, the v0.8 evaluator, and every rule below.

v0.8 preserves the v0.5 manifests and external success predicates. It fixes a
Reacher runtime-ordering error: while CLEAR owns success scoring, the evaluator
must also disable the underlying dm-control task termination so an inner
`action_repeat` step cannot emit `LAST` and auto-reset before external scoring.
The override must remain active after both `reset()` and model recompilation.
...
Reference planning comparisons must pass `--actor-warmstart off` and retain the
runtime source hashes written by CLEAR-LeWM.
...
```text
manifests/v0.8/<task>/<moderate|strict>-seed<seed>-n100.json
```
...
The v0.8 manifest files are byte-identical to v0.5 because pair selection and
external predicates did not change. v0.5 results remain immutable historical
evidence and are not relabeled as v0.8 results.
