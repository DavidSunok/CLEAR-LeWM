# CLEAR-LeWM v0.9 canonical manifests

v0.9 keeps the v0.8 pair selection, task predicates, and Reacher runtime
correction. It changes initialization to rest-start: the recorded configuration
and goal are preserved, while dynamic velocity is set to zero before the first
policy observation. Later replans use up to three real observations and the
corresponding executed action blocks collected during evaluation. It contains
two complementary protocols for all four tasks and three fixed evaluation
seeds:

```text
manifests/v0.9/<task>/<moderate|strict>-seed<0|1|42>-n100.json
```
...
The selected pairs are unchanged from v0.8, but the manifest schema and runtime
initialization are not. v0.8 results remain immutable historical evidence and
are not comparable to v0.9 results as matched benchmark numbers.
