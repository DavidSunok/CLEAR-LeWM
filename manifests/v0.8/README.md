# CLEAR-LeWM v0.8 canonical manifests

v0.8 keeps the v0.5 pair selection and task predicates, and adds the versioned
Reacher runtime correction that prevents dm-control from auto-resetting inside
an action-repeat before CLEAR scores the external policy step. It contains two
complementary protocols for all four tasks and three fixed evaluation seeds:

```text
manifests/v0.8/<task>/<moderate|strict>-seed<0|1|42>-n100.json
```
...
The manifest JSON files are byte-identical to v0.5 because pair selection and
external scoring predicates did not change. v0.5 remains immutable historical
evidence; results produced with the corrected Reacher runtime are labeled v0.8.
